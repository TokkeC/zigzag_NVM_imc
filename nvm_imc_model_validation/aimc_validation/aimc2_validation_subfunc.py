...
def aimc2_cost_estimation(aimc, cacti_value):
    ...
    """
    col_mux = 2
    mults = MultiplierArray(vdd=aimc['vdd'], input_precision=int(aimc['multiplier_precision']),
                            number_of_multiplier=col_mux * input_channel, unit_area=unit_area, unit_delay=unit_delay,
                            unit_cap=unit_cap)
    ...
    adder1 = Adder(vdd=aimc['vdd'], input_precision=7, unit_area=unit_area, unit_delay=unit_delay,
                   unit_cap=unit_cap)  # 8 in total
    adder2 = Adder(vdd=aimc['vdd'], input_precision=9, unit_area=unit_area, unit_delay=unit_delay,
                   unit_cap=unit_cap)  # 4 in total
    adder3 = Adder(vdd=aimc['vdd'], input_precision=12, unit_area=unit_area, unit_delay=unit_delay,
                   unit_cap=unit_cap)  # 2 in total
    adder4 = Adder(vdd=aimc['vdd'], input_precision=15, unit_area=unit_area, unit_delay=unit_delay,
                   unit_cap=unit_cap)  # 1 in total
    adder_tree = AdderTree(vdd=aimc['vdd'], input_precision=int(aimc['adder_input_precision']),
                           number_of_input=input_channel, unit_area=unit_area, unit_delay=unit_delay, unit_cap=unit_cap)

    """
    accumulator for each output channel
    """
    accumulator = Adder(vdd=aimc['vdd'], input_precision=int(aimc['accumulator_precision']), unit_area=unit_area,
                        unit_delay=unit_delay, unit_cap=unit_cap)
    ...
    """
    unitbank = MemoryInstance(name='unitbank', size=aimc['rows'] * aimc['cols'], r_bw=aimc['cols'], w_bw=aimc['cols'],
                              delay=cacti_value['delay'] * 0, r_energy=cacti_value['r_energy'],
                              w_energy=cacti_value['w_energy'], area=cacti_value['area'], r_port=1, w_port=1, rw_port=0,
                              latency=0)
    regs_accumulator = MemoryInstance(name='regs_accumulator', size=aimc['reg_accumulator_precision'],
                                      r_bw=aimc['reg_accumulator_precision'], w_bw=aimc['reg_accumulator_precision'],
                                      delay=unit_reg.calculate_delay(), r_energy=0,
                                      w_energy=unit_reg.calculate_cap() * aimc['vdd'] ** 2 * aimc[
                                          'reg_accumulator_precision'],
                                      area=unit_reg.calculate_area() * aimc['reg_accumulator_precision'], r_port=1,
                                      w_port=1, rw_port=0, latency=0)
    regs_pipeline = MemoryInstance(name='regs_pipeline', size=5 * 16, r_bw=5 * 16, w_bw=5 * 16, delay=0, r_energy=0,
                                   w_energy=unit_reg.calculate_cap() * aimc['vdd'] ** 2 * 5 * 16,
                                   area=unit_reg.calculate_area() * 5 * 16, r_port=1, w_port=1, rw_port=0, latency=1)

    energy_wl = aimc['input_channel'] * aimc['unit_cap'] / 2 * 2 * aimc['vdd'] ** 2 * aimc[
        'weight_precision']  # per output channel
    ...
    energy_en = aimc['input_channel'] * aimc['unit_cap'] / 2 * 2 * aimc[
        'vdd'] ** 2  # per output channel (energy cost on "en" enable signal)
    ...
    area_adder_tree = aimc['banks'] * aimc['output_channel'] * (
                8 * adder1.calculate_area() + 4 * adder2.calculate_area() + 2 * adder3.calculate_area() + 1 * adder4.calculate_area())
    ...
    predicted_delay = max(2 * (unitbank.delay + mults.calculate_delay() + adc.calculate_delay()),
    ...
    energy_adder_tree = (1 - aimc['weight_sparsity']) * aimc['banks'] * aimc[
        'output_channel'] * (
                                    8 * adder1.calculate_energy() + 4 * adder2.calculate_energy() + 2 * adder3.calculate_energy() + 1 * adder4.calculate_energy())
    energy_accumulator = aimc['banks'] * aimc['output_channel'] * accumulator.calculate_energy()
    energy_banks = (1 - aimc['weight_sparsity']) * aimc['banks'] * aimc['output_channel'] * (
    ...
    energy_adc = (1 - aimc['weight_sparsity']) * aimc['banks'] * aimc['output_channel'] * 16 * adc.calculate_energy(
        vdd=aimc['vdd'])
    energy_dac = aimc['banks'] * 2 * aimc['input_channel'] * dac.calculate_energy(vdd=aimc['vdd'],
                                                                                  k0=aimc['dac_energy_k0'])
