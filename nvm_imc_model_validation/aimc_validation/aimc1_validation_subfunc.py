...
def aimc1_cost_estimation(aimc, cacti_value):
    ...
    energy_bl = aimc['input_channel'] * aimc['unit_cap'] / 2 * 2 * aimc[
        'vdd'] ** 2  # per output channel (aimc['unit_cap']/2 for bitline cap/cell, *2 for 2 bitline port of 2 cells connecting together)
    energy_en = aimc['input_channel'] * aimc['unit_cap'] / 2 * aimc[
        'vdd'] ** 2  # per output channel (energy cost on "csbias" enable signal)
    ...
    energy_adc = (1 - aimc['weight_sparsity']) * aimc['banks'] * aimc['output_channel'] * adc.calculate_energy(
        vdd=aimc['vdd'])
