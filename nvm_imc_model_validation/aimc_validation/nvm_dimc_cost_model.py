"""
    Taken from validation of IMC-zigzag

    It holds the class definitions for seperate gates or digital blocks.
    Every class itself however does have an input about the 'unit' of area delay and capacitance (parasitic).
    Depending on the complexity of the gate it's just simply once this, or a formula regarding them.
"""
...
class UnitDff:
    ...
    def __init__(self, unit_area: float, unit_delay: float, unit_cap: float):
        ...
        self.delay = 0 # On the clock itself +-
        self.cap = unit_cap * 3
...
# """
#     For SRAM, the array is seen as multiple columns which are all seperate multipliers.
#     For ReRAM, I try to do the same, but I need seperate classes to define ReRAM parameters/specifications instead of the standard SRAM ones.
# """
# class ReRAMMultiplier:
#     def __init__(self, vdd: float, input_precision: int, unit_area: float, unit_delay: float, unit_cap: float):
#         """
#         Class for a single RERAM multiplier that performs 1 bit x multiple bits
#         :param vdd:              The supply voltage (unit: V)
#         :param input_precision:  The bit precision of the input (unit: bit)
#         :param output_precision: The bit precision of the output (unit: bit)
#         """
...
"""
    Right now this is for SRAM, and it is used to calculate the performance of a ROW in the SRAM CIM ARRAY --> NEEDS TO BE ReRAM NOW
"""
class MultiplierArray:
    def __init__(self, vdd: float, input_precision: int, number_of_multiplier: int, unit_area: float, unit_delay: float,
                 unit_cap: float):
        """
        Class for a single multiplier that performs 1 bit x multiple bits
        :param vdd:                  The supply voltage (unit: V)
...
# """
#     First version of MultiplierArray Class for ReRAM
# """
# class ReRAMMultiplierArray:
...
#                  unit_cap: float):
#         """
#         Class for a single COLUMN of the ReRAM ARRAY, seen as a MULTIPLIER, that performs 1 bit x multiple bits
#         :param vdd:                  The supply voltage (unit: V)
#         :param input_precision:      The bit precision of the input (unit: bit)
#         :param output_precision:     The bit precision of the output (unit: bit)
#         :param number_of_multiplier: The number of multiplier, AMOUNT OF INPUTS TO COLUMN (so amount of rows)
#         """
#         self.ReRAMmult = ReRAMMultiplier(vdd, input_precision, unit_area, unit_delay, unit_cap)
#         self.vdd = vdd
#         self.input_precision = input_precision
#         self.output_precision = input_precision  # output precision = input precision
#         self.number_of_multiplier = number_of_multiplier
...
#         area: The area cost (unit: mm2)
#         """
#         area = self.ReRAMmult.calculate_area() * self.number_of_multiplier
#         return area
#
#     def calculate_delay(self):
#         """
#         delay: The delay cost (unit: ns)
#         """
#         delay = self.ReRAMmult.calculate_delay() # Just 1 time the delay, as they work all in Parallel!!!
#         return delay
#
#     def calculate_energy(self):
#         """
#         energy: The energy cost (unit: fJ)
#         """
#         energy = self.ReRAMmult.calculate_energy() * self.number_of_multiplier #Energy DOES scale
#         return energy
    ...
    def calculate_delay_msb(self):
        ...
        delay_carry = (self.xor2.calculate_delay() + 2 * self.nand2.calculate_delay()) + (
                    2 * self.nand2.calculate_delay()) * (
                                  self.input_precision - 1)  # A-to-Cout -> Cin-to-Count * (precision-1)
        return delay_carry
...
class AdderTree:
    def __init__(self, vdd: float, input_precision: int, number_of_input: int, unit_area: float, unit_delay: float,
                 unit_cap: float):
        """
    ...
    def calculate_area(self):
        ...
        area = self.number_of_1b_adder * Adder(vdd=self.vdd, input_precision=1, unit_area=self.unit_area,
                                               unit_delay=self.unit_delay, unit_cap=self.unit_cap).calculate_area()
        return area
    ...
    def calculate_delay(self):
        ...
        delay: The delay cost (unit: ns)
        """
        last_adder = Adder(vdd=self.vdd, input_precision=self.output_precision - 1, unit_area=self.unit_area,
                           unit_delay=self.unit_delay, unit_cap=self.unit_cap)
    ...
    def calculate_energy(self):
        ...
        energy: The energy cost (each time it is triggered) (unit: fJ)
        """
        energy = self.number_of_1b_adder * Adder(vdd=self.vdd, input_precision=1, unit_area=self.unit_area,
                                                 unit_delay=self.unit_delay, unit_cap=self.unit_cap).calculate_energy()
        return energy


class MemoryInstance:
    """
    class for: regs (input regs, output regs), memory bank (copy from Zigzag code, with area, delay added)
    """

    def __init__(self, name: str, size: int, r_bw: int, w_bw: int, delay: float, r_energy: float, w_energy: float,
                 area: float,
